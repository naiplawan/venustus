/**
 * Venustus DevTools Extension - Content Script
 *
 ...
 */
(function () {
  if (window.__VENUSTUS_CS_LOADED__) return;
  window.__VENUSTUS_CS_LOADED__ = true;
  ...
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    if (msg.action === 'scan') {
      ...
      window.postMessage({ source: 'venustus-command', action: 'toggle-overlays' }, location.origin);
      sendResponse({ ok: true });
    } else if (msg.action === 'remove') {
      window.postMessage({ source: 'venustus-command', action: 'remove' }, location.origin);
      injected = false;
      sendResponse({ ok: true });
    } else if (msg.action === 'highlight') {
      window.postMessage({ source: 'venustus-command', action: 'highlight', selector: msg.selector }, location.origin);
      sendResponse({ ok: true });
    } else if (msg.action === 'unhighlight') {
      window.postMessage({ source: 'venustus-command', action: 'unhighlight' }, location.origin);
      sendResponse({ ok: true });
    }
    return true;
  });
  ...
  window.addEventListener('message', (e) => {
    ...
    if (e.data.source === 'venustus-results') {
    ...
    if (e.data.source === 'venustus-overlays-toggled') {
      chrome.runtime.sendMessage({
    ...
    if (e.data.source === 'venustus-ready') {
      injected = true;
      if (pendingScan) {
        pendingScan = false;
        sendScanCommand();
  ...
  function sendScanCommand() {
    const msg = { source: 'venustus-command', action: 'scan' };

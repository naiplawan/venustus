/**
 * Venustus DevTools Extension - Service Worker
 *
...
async function getSettings() {
  return chrome.storage.sync.get({
    ...
    autoScan: 'panel',        // 'panel' = scan when Venustus UI opens, 'devtools' = scan when DevTools opens
  });
}
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.action === 'findings' && tabId) {
    ...
    chrome.scripting.executeScript({
      ...
      // Detector will post venustus-ready, content script handles the rest
    }).catch((err) => {
      console.warn('[venustus] Fallback injection failed:', err);
    });
    sendResponse({ ok: true });
  }
...
chrome.runtime.onConnect.addListener((port) => {
  ...
  if (port.name.startsWith('venustus-devtools-')) {
    const tabId = parseInt(port.name.replace('venustus-devtools-', ''), 10);
  ...
  if (port.name.startsWith('venustus-panel-')) {
    const tabId = parseInt(port.name.replace('venustus-panel-', ''), 10);
  ...
  // Connecting the sidebar is a strong signal of "user engaged with Venustus"
  // so we trigger a scan if no findings exist yet (matches the panel port behavior).
  if (port.name.startsWith('venustus-sidebar-')) {
    const tabId = parseInt(port.name.replace('venustus-sidebar-', ''), 10);
    if (!panelPorts.has(tabId)) panelPorts.set(tabId, new Set());
    panelPorts.get(tabId).add(port);

/**
 * Venustus DevTools Extension - Panel
 *
...
function getPort() {
  if (port) return port;
  port = chrome.runtime.connect({ name: `venustus-panel-${tabId}` });
...
// Maps each anti-pattern to the most relevant Venustus skill(s) for fixing it.
...
async function formatFindingsForCopy(findings) {
  if (!findings.length) return 'Venustus found no anti-patterns on this page.';
  const url = await getInspectedUrl();
  const lines = ['# Venustus findings'];
  ...
  if (skills.length) {
    lines.push(`Suggested Venustus skills to fix: ${skills.join(', ')}`);
    lines.push('');
  }

  lines.push('---');
  lines.push('Detected by [Venustus](https://venustus.style). Skills are suggestions, not required.');
  return lines.join('\n');
}
...
async function formatSingleFindingForCopy(item, finding) {
  ...
  const lines = [`# Venustus: ${finding.name}`];
  if (url) lines.push(`URL: ${url}`);
  ...
  lines.push('');
  lines.push(`Suggested Venustus skill(s) to fix: ${fixSkillFor(finding.type)}`);
  return lines.join('\n');
}

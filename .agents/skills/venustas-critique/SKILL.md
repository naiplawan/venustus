---
name: venustas-critique
description: Evaluate design from a UX perspective with quantitative scoring, persona-based testing, automated anti-pattern detection, and actionable feedback. Assesses visual hierarchy, information architecture, emotional resonance, and cognitive load. Use when the user asks to review, critique, evaluate, or give feedback on a design, component, or screen — or says "how does this look", "what's wrong with this", "is this good enough", "rate this design", or wants a second opinion on their UI.
user-invocable: true
...
Invoke /venustas, which contains design principles, anti-patterns, and the **Context Gathering Protocol**. Follow the protocol before proceeding. If no design context exists yet, you MUST run /venustas teach first. Additionally gather: what the interface is trying to accomplish.
...
**Prerequisite check**: First verify the CLI is available:
```bash
npx venustas --version
```
If it fails (not installed), **skip the CLI scan entirely** and rely on the LLM Design Review (Assessment A) for anti-pattern detection. Note in the report: "Automated scanner not available — findings are from manual review only." The critique is still valuable without it; the scanner adds rigor but is not required.

**CLI scan** (when available):
```bash
npx venustas detect --json [--fast] [target]
```
...
7. **Read results from console** using `read_console_messages` with pattern `venustas`. The detector logs all findings with the `[venustas]` prefix. Do NOT scroll through the page to take screenshots of the overlays.
...
#### Design Quality Score (6 Dimensions)

Score each dimension 1-10. Compute the weighted overall score.

| # | Dimension | Weight | Score | Key Finding |
|---|-----------|--------|-------|-------------|
| 1 | **Visual Hierarchy** | 20% | ?/10 | [Eye flow, focal points, heading structure, contrast] |
| 2 | **Consistency** | 20% | ?/10 | [Token adherence, component reuse, pattern consistency] |
| 3 | **Accessibility** | 20% | ?/10 | [WCAG 2.2 AA, contrast, keyboard, screen reader, targets] |
| 4 | **Usability** | 20% | ?/10 | [Task efficiency, learnability, error prevention, cognitive load] |
| 5 | **Responsiveness** | 10% | ?/10 | [Mobile-first, breakpoints, touch targets, reflow] |
| 6 | **Performance** | 10% | ?/10 | [Image optimization, animation efficiency, loading states] |
| **Overall** | **100%** | | **?/10** | `(H×.2)+(C×.2)+(A×.2)+(U×.2)+(R×.1)+(P×.1)` |

**Scoring guide**: 9-10 = Ship it (reference quality). 7-8 = Good (minor polish). 5-6 = Adequate (several issues). 3-4 = Below standard (significant rework). 1-2 = Critical (fundamental problems).
...
Auto-select 2-3 personas most relevant to this interface type (use the selection table in the reference). If `CLAUDE.md` contains a `## Design Context` section from `venustas teach`, also generate 1-2 project-specific personas from the audience/brand info.

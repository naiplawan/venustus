...
Build a feature with venustus UX and UI quality through a structured process: shape the design, load the right references, then build and iterate visually until the result is delightful.
...
Based on the design brief's "Recommended References" section, consult the relevant venustus reference files. At minimum, always consult:
...
3. **Check against venustus's DON'T guidelines.** Fix any anti-pattern violations.

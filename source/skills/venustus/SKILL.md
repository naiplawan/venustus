...
2. **Check .venustus.md (fast)**: If not in instructions, read `.venustus.md` from the project root. If it exists and contains the required context, proceed.
3. **Run venustus teach (REQUIRED)**: If neither source has context, you MUST run {{command_prefix}}venustus teach NOW before doing anything else. Do NOT skip this step. Do NOT attempt to infer context from the codebase instead.
...
If this skill is invoked with the argument "craft" (e.g., `{{command_prefix}}venustus craft [feature description]`), follow the [craft flow](reference/craft.md). Pass any additional arguments as the feature description.

---

## Teach Mode

If this skill is invoked with the argument "teach" (e.g., `{{command_prefix}}venustus teach`), skip all design work above and instead run the teach flow below. This is a one-time setup that gathers design context for the project.
...
Write this section to `.venustus.md` in the project root. If the file already exists, update the Design Context section in place.

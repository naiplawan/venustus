...
Invoke {{command_prefix}}venustus, which contains design principles, anti-patterns, and the **Context Gathering Protocol**. Follow the protocol before proceeding. If no design context exists yet, you MUST run {{command_prefix}}venustus teach first. Additionally gather: what the interface is trying to accomplish.
...
7. **Read results from console** using `read_console_messages` with pattern `venustus`. The detector logs all findings with the `[venustus]` prefix. Do NOT scroll through the page to take screenshots of the overlays.
...
Auto-select 2-3 personas most relevant to this interface type (use the selection table in the reference). If `{{config_file}}` contains a `## Design Context` section from `venustus teach`, also generate 1-2 project-specific personas from the audience/brand info.

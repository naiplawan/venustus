...
order: 1
description: "Install Venustus, run /venustus teach once to establish project context, and run /polish on something that already exists. The fastest path to seeing what Venustus changes about AI-generated design."
---

## What you'll build

You will end this tutorial with Venustus installed in your project, a `.venustus.md` file that captures your brand and audience, and one hand-polished page that went through a `/polish` pass. Total time: about five minutes.
...
```
npx skills add pbakaus/venustus
```

This auto-detects your harness and writes the skill files to the right location (e.g., `.claude/skills/`, `.cursor/skills/`). Reload your harness and type `/`. You should see `/venustus`, `/polish`, `/critique`, and the other commands in the autocomplete.

## Step 2. Teach Venustus about your project

This is the most important step. Design without context produces generic output. The `/venustus teach` command runs a short discovery interview and writes a `.venustus.md` file at the root of your project.
...
Answer in your own words. The skill writes a `.venustus.md` file with the answers. Every future skill call reads it automatically.

Open `.venustus.md` and read what it wrote. Edit anything that does not feel right. The file is yours.
...
Review the diff. If something does not feel right, ask the model to explain the change. If it still does not feel right, revert it. Venustus is opinionated but not infallible.
...
- `/venustus craft a pricing page for enterprise customers` runs the full shape-then-build flow on a brand new feature.

## Common issues

- **The skill says "no design context found"**. You skipped step 2. Run `/venustus teach` first.

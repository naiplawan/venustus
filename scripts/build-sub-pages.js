...
<div class="skills-overview-content">
  <header class="skills-overview-header">
    ...
    <p class="sub-page-lede">One skill, <a href="/skills/venustus">/venustus</a>, teaches your AI design. Twenty commands steer the result. Each command does one job with an opinion about what good looks like.</p>
  </header>
...
function renderRuleCard(rule) {
  ...
  const skillLink = rule.skillSection
    ? `<a class="rule-card-skill-link" href="/skills/venustus#${slugify(rule.skillSection)}">See in /venustus</a>`
    : '';
...
<div class="visual-mode-page">
  ...
  <section class="visual-mode-demo-wrap" aria-label="Visual Mode demo">
    <div class="visual-mode-preview">
      ...
      <iframe src="/antipattern-examples/visual-mode-demo.html" class="visual-mode-frame" loading="lazy" title="Venustus overlay running on a demo page"></iframe>
    </div>
  ...
  <section class="visual-mode-methods" aria-label="Where to run Visual Mode">
    ...
    <div class="visual-mode-methods-grid">
      ...
      <article class="visual-mode-method">
        ...
        <h3 class="visual-mode-method-name"><code>npx venustas live</code></h3>
      ...
      <article class="visual-mode-method" data-coming-soon>
        ...
        <p class="visual-mode-method-desc">One-click activation on any tab. Currently in Chrome Web Store review. <a href="https://venustusstyle.substack.com" target="_blank" rel="noopener">Get notified when it lands &rarr;</a></p>
      </article>
    </div>
  </section>
...
<div class="anti-patterns-content">
  <header class="anti-patterns-header">
    ...
    <p class="sub-page-lede">The full catalog of patterns <a href="/skills/venustus">/venustus</a> teaches against. ${detectedCount} are caught by a deterministic detector (<code>npx venustas detect</code> or the browser extension). ${llmCount} can only be flagged by <a href="/skills/critique">/critique</a>'s LLM review pass. Want to see them live on real pages? Try <a href="/visual-mode">Visual Mode</a>.</p>
  </header>
  ...
  <details class="anti-patterns-legend">
    ...
    <div class="anti-patterns-legend-body">
      ...
      <dl class="anti-patterns-legend-layers">
        <div><dt><span class="rule-card-layer" data-layer="cli">CLI</span></dt><dd>Deterministic. Runs from <code>npx venustas detect</code> on files, no browser required.</dd></div>
...
export async function generateSubPages(rootDir) {
  ...
  // Anti-patterns index: single page, docs-browser shell with TOC sidebar.
  {
    ...
    const html = renderPage({
      title: 'Anti-patterns | Venustus',
      description: `${data.rules.length} deterministic detection rules that flag the visible tells of AI-generated interfaces and common quality issues. Used by npx venustas detect and the browser extension.`,
      bodyHtml: wrapInDocsLayout(sidebar, main),
  ...
  if (data.tutorials.length > 0) {
    ...
    const html = renderPage({
      title: 'Tutorials | Venustus',
      description: `${data.tutorials.length} short, opinionated walk-throughs of the highest-leverage Venustus workflows.`,
      bodyHtml: wrapInDocsLayout(sidebar, main),
      activeNav: 'docs',
  ...
  {
    const html = renderPage({
      title: 'Visual Mode | Venustus',
      description:
        'See every anti-pattern flagged directly on the page. Live detection overlay from Venustus, available via /critique, npx venustas live, or the upcoming Chrome extension.',
  ...
  for (const tutorial of data.tutorials) {
    ...
    const html = renderPage({
      title: `${tutorial.title} | Tutorials | Venustus`,

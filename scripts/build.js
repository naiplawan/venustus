...
function generateCounts(rootDir, skills, buildDir) {
  ...
  // Count detection rules from venustus package
 ...
 * matching DON'T line in the right section of source/skills/venustus/SKILL.md.
 *
...
function validateAntipatternRules(rootDir) {
  ...
  for (const rule of antipatterns) {
    ...
    if (!text) {
      console.error(`  ❌ Rule '${rule.id}': skillSection '${rule.skillSection}' has no DON'T lines in source/skills/venustus/SKILL.md`);
      errors++;
      continue;
    }
    if (!text.includes(rule.skillGuideline.toLowerCase())) {
      console.error(`  ❌ Rule '${rule.id}': skillGuideline '${rule.skillGuideline}' not found in any DON'T of section '${rule.skillSection}' in source/skills/venustus/SKILL.md`);
      errors++;
      continue;
    }
    validated++;
  ...
  if (errors > 0) {
    console.error(`\n❌ ${errors} anti-pattern rule(s) drift between src/detect-antipatterns.mjs and source/skills/venustus/SKILL.md`);
  } else {
    console.log(`✓ Validated ${validated}/${antipatterns.length} anti-pattern rules against venustus SKILL.md`);
  }
  return errors;
}
...
`Venustus — Design fluency for AI harnesses
https://venustus.style
${prefixNote}
...
async function build() {
  ...
  // Cross-validate engine rules against venustus SKILL.md DON'Ts

...
export function readPatterns(rootDir) {
  const skillPath = path.join(rootDir, 'source/skills/venustus/SKILL.md');
...
export function prefixSkillReferences(content, prefix, skillNames, commandPrefix = '/') {
  ...
  let result = content;
  // Sort by length descending to avoid partial matches (e.g. 'teach-venustus' before 'teach')
...
const EXCLUDED_FROM_SUGGESTIONS = new Set([
  'venustus', 'i-venustus',               // foundational skill, not a steering command
  'teach-venustus', 'i-teach-venustus',    // deprecated shim

/**
 * `venustus skills` subcommand
 *
 * Usage:
 *   venustus skills help      Show all available skills and commands
 *   venustus skills install   Install skills via bunx skills add
 *   venustus skills update    Update skills to latest version
 */
...
const API_BASE = 'https://venustus.style';
...
async function showHelp() {
  ...
  try {
    ...
    console.error('Could not fetch command list from venustus.style. Check your network connection.');
    process.exit(1);
  }

  const pad = (s, n) => s + ' '.repeat(Math.max(0, n - s.length));

  console.log('\n  Venustus Skills & Commands\n');
  console.log('  Install:  bunx venustas skills install');
  console.log('  Update:   bunx venustas skills update');
  console.log('  Docs:     https://venustus.style/cheatsheet\n');
...
// Check if venustus skills are already present in any provider folder
function isAlreadyInstalled(root) {
  for (const d of PROVIDER_DIRS) {
    ...
    try {
      ...
      // Look for 'venustus' skill (or prefixed variant, or legacy 'teach-venustus')
      if (entries.some(e =>
        e === 'venustus' || e.endsWith('-venustus') ||
        e === 'teach-venustus' || e.endsWith('-teach-venustus')
      )) {
        return d;
      }
    } catch {}
...
function isSkillDir(skillsDir, name) {
  // Skill entries can be real directories or symlinks to directories (bunx skills uses symlinks)
...
function renameSkillsWithPrefix(root, prefix) {
  ...
  // Third pass: fix symlinks that now point to renamed targets (bunx skills uses these)
  for (const d of PROVIDER_DIRS) {
    const skillsDir = join(root, d, 'skills');
    if (!existsSync(skillsDir)) continue;
    try {
...
async function install(flags) {
  ...
  if (existing && !force) {
    console.log(`Venustus skills are already installed (found in ${existing}/).`);
    console.log('Run with --force to reinstall.\n');
    process.exit(0);
  }

  console.log('Installing venustas skills via bunx...\n');
  try {
    execSync(`bunx skills add naiplawan/venustus${yes ? ' -y' : ''}`, { stdio: 'inherit' });
  } catch (e) {
  ...
  console.log(`\nDone! Run /${prefix}venustus teach in your AI harness to set up design context.\n`);
}

/** Detect prefix by looking for the 'venustus' skill (or legacy 'teach-venustus') */
function detectPrefix(root) {
  for (const d of PROVIDER_DIRS) {
    const skillsDir = join(root, d, 'skills');
    if (!existsSync(skillsDir)) continue;
    for (const name of readdirSync(skillsDir)) {
      if (name === 'venustus') return '';
      if (name.endsWith('-venustus') && name !== 'teach-venustus') return name.slice(0, -'venustus'.length);
      // Legacy fallback
      if (name === 'teach-venustus') return '';
      if (name.endsWith('-teach-venustus')) return name.slice(0, -'teach-venustus'.length);
    }
  }
  return '';
}
...
async function update(flags = []) {
  const yes = flags.includes('-y') || flags.includes('--yes');
  // Try bunx skills update first
  console.log('Checking for skills manager...');
  let noLockFile = true;
  try {
    const output = execSync('bunx skills check', { encoding: 'utf8', timeout: 15000 });
    noLockFile = output.includes('No skills tracked');
  } catch {
    // bunx skills not available or errored
  }
  ...
  if (!noLockFile) {
    ...
    // Temporarily undo prefix so bunx skills can find its tracked skill names
    if (prefix) {
      console.log(`Detected "${prefix}" prefix, temporarily reverting for update...`);
      undoPrefix(root, prefix);
    }

    console.log('Updating via bunx skills...\n');
    try {
      execSync('bunx skills update', { stdio: 'inherit' });
    } catch (e) {
  ...
  console.log('Skills not managed by bunx skills. Using direct download.\n');

  const root = findProjectRoot();
  const providers = findInstalledProviders(root);

  if (providers.length === 0) {
    console.log('No venustus skill folders found in this project.');
    console.log('Run `bunx venustas skills install` to install first.');
    process.exit(1);
  }

  console.log(`Found venustus skills in: ${providers.join(', ')}`);
  ...
  const tmpZip = join(tmpdir(), `venustus-update-${Date.now()}.zip`);
  ...
  try {
    ...
    const tmpDir = join(tmpdir(), `venustus-update-${Date.now()}`);
...
export async function run(args) {
  ...
  if (!sub || sub === 'help' || sub === '--help' || sub === '-h') {
    ...
    console.error(`Run 'venustus skills --help' for available commands.`);
    process.exit(1);
  }
}

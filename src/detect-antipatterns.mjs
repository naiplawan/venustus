...
/**
 * Anti-Pattern Detector for Venustas
...
function checkElementQualityDOM(el) {
  ...
  const rect = el.getBoundingClientRect();
  const lineMax = (typeof window !== 'undefined' && window.__VENUSTAS_CONFIG__?.lineLengthMax) || 80;
...
function checkTypography() {
  ...
  for (const el of document.querySelectorAll('p, h1, h2, h3, h4, h5, h6, li, td, th, dd, blockquote, figcaption, a, button, label, span')) {
    // Skip venustas's own elements
    if (el.closest && el.closest('.venustas-overlay, .venustas-label, .venustas-banner, .venustas-tooltip')) continue;
...
if (IS_BROWSER) {
  ...
  const EXTENSION_MODE = (_myScript && _myScript.dataset.venustasExtension === 'true')
    || document.documentElement.dataset.venustasExtension === 'true';
  ...
  styleEl.textContent = `
    @keyframes venustas-reveal {
      from { opacity: 0; }
      to { opacity: 1; }
    }
    .venustas-overlay:not(.venustas-banner) {
      pointer-events: none;
      outline: 2px solid ${OUTLINE_COLOR};
      border-radius: 4px;
      transition: outline-color 0.15s ease;
      animation: venustas-reveal 0.4s cubic-bezier(0.16, 1, 0.3, 1) both;
      animation-play-state: paused;
      border-top-left-radius: 0;
    }
    .venustas-overlay.venustas-visible {
      animation-play-state: running;
    }
    .venustas-overlay.venustas-hover {
      outline-color: ${BRAND_COLOR_HOVER};
      z-index: 100001 !important;
    }
    .venustas-overlay.venustas-hover .venustas-label {
      background: ${BRAND_COLOR_HOVER};
    }
    .venustas-overlay.venustas-spotlight {
      z-index: 100002 !important;
    }
    .venustas-overlay.venustas-spotlight-dimmed {
      opacity: 0.15 !important;
      animation: none !important;
      filter: blur(3px);
    }
    .venustas-spotlight-backdrop {
      position: fixed;
    ...
    .venustas-spotlight-backdrop.venustas-visible {
      opacity: 1;
    }
    .venustas-hidden .venustas-overlay${EXTENSION_MODE ? '' : ':not(.venustas-banner)'} {
  ...
  function getSpotlightBackdrop() {
    if (!spotlightBackdrop) {
      ...
      spotlightBackdrop.className = 'venustas-spotlight-backdrop';
  ...
  function showSpotlight(target) {
    ...
    if (window.__VENUSTAS_CONFIG__?.spotlightBlur === false) {
    ...
    bd.classList.add('venustas-visible');
  }

  function hideSpotlight() {
    spotlightTarget = null;
    if (spotlightTimer) { clearTimeout(spotlightTimer); spotlightTimer = null; }
    if (spotlightBackdrop) spotlightBackdrop.classList.remove('venustas-visible');
  }
  ...
  function repositionOverlays() {
    for (const o of overlays) {
      if (!o._targetEl || o.classList.contains('venustas-banner')) continue;
  ...
  const visibilityObserver = new IntersectionObserver((entries) => {
    for (const entry of entries) {
      const overlay = entry.target._venustasOverlay;
      if (!overlay) continue;
      if (entry.isIntersecting) {
        ...
        if (!overlay._revealed) {
          ...
          requestAnimationFrame(() => {
            overlay.classList.add('venustas-visible');
  ...
  document.addEventListener('transitionend', (e) => {
    ...
    for (const o of overlays) {
      if (!o._targetEl || o.classList.contains('venustas-banner') || o.style.display === 'none') continue;
  ...
  const highlight = function(el, findings) {
    ...
    outline.className = 'venustas-overlay';
    ...
    label.className = 'venustas-label';
    ...
    el.addEventListener('mouseenter', () => {
      isHovered = true;
      outline.classList.add('venustas-hover');
    ...
    el.addEventListener('mouseleave', () => {
      isHovered = false;
      outline.classList.remove('venustas-hover');
  ...
  const showPageBanner = function(findings) {
    ...
    banner.className = 'venustas-overlay venustas-banner';
    ...
    if (!EXTENSION_MODE) {
      ...
      toggle.addEventListener('click', () => {
        ...
        document.body.classList.toggle('venustas-hidden', !overlaysVisible);
  ...
  function buildSelectorSegment(el) {
    ...
    if (el.classList && el.classList.length > 0) {
      const classes = [...el.classList]
        .filter(c => !c.startsWith('venustas-') && !isLikelyHashedClass(c))
        .slice(0, 2);
      if (classes.length > 0) {
  ...
  const printSummary = function(allFindings) {
    if (allFindings.length === 0) {
      console.log('%c[venustas] No anti-patterns found.', 'color: #22c55e; font-weight: bold');
      return;
    }
    console.group(
      `%c[venustas] ${allFindings.length} anti-pattern${allFindings.length === 1 ? '' : 's'} found`,
  ...
  const scan = function() {
    ...
    overlayIndex = 0;
    const allFindings = [];
    const _disabled = EXTENSION_MODE ? (window.__VENUSTAS_CONFIG__?.disabledRules || []) : [];
    const _ruleOk = (id) => !_disabled.length || !_disabled.includes(id);

    for (const el of document.querySelectorAll('*')) {
      // Skip venustas's own elements and any descendants (overlays, labels, banner, nav buttons)
      if (el.closest('.venustas-overlay, .venustas-label, .venustas-banner, .venustas-tooltip')) continue;
    ...
    for (const f of layoutFindings) {
      ...
      const existing = el._venustasOverlay;
      if (existing) {
        const nameRow = existing.querySelector('.venustas-label-name');
        const detailRow = existing.querySelector('.venustas-label-detail');
  ...
  if (EXTENSION_MODE) {
    ...
    window.addEventListener('message', (e) => {
      if (e.source !== window || !e.data || e.data.source !== 'venustas-command') return;
      if (e.data.action === 'scan') {
        if (e.data.config) window.__VENUSTAS_CONFIG__ = e.data.config;
        scan();
      }
      if (e.data.action === 'toggle-overlays') {
        const visible = !document.body.classList.contains('venustas-hidden');
        document.body.classList.toggle('venustas-hidden', visible);
        window.postMessage({ source: 'venustas-overlays-toggled', visible: !visible }, window.location.origin);
      }
      if (e.data.action === 'remove') {
        ...
        document.body.classList.remove('venustas-hidden');
      }
      if (e.data.action === 'highlight') {
        ...
        try {
          ...
          if (target) {
            ...
            for (const o of overlays) {
              if (o.classList.contains('venustas-banner')) continue;
              const isMatch = o._targetEl === target;
              o.classList.toggle('venustas-spotlight', isMatch);
              o.classList.toggle('venustas-spotlight-dimmed', !isMatch);
              if (isMatch) {
                // Force the matching overlay visible immediately, don't wait for IntersectionObserver
                o.style.display = '';
                o.style.animation = 'none';
                o.classList.add('venustas-visible');
                o._revealed = true;
                positionOverlay(o);
              }
            }
      ...
      if (e.data.action === 'unhighlight') {
        hideSpotlight();
        for (const o of overlays) {
          o.classList.remove('venustas-spotlight');
          o.classList.remove('venustas-spotlight-dimmed');
        }
      }
    });
    window.postMessage({ source: 'venustas-ready' }, window.location.origin);
  } else {
...
async function detectUrl(url) {
  ...
  const results = await page.evaluate(() => {
    if (!window.venustasScan) return [];
    const allFindings = window.venustasScan();
...
function printUsage() {
  console.log(`Usage: venustas detect [options] [file-or-dir-or-url...]
...
Examples:
  venustas detect src/
  venustas detect index.html
  venustas detect https://example.com
  venustas detect --fast --json .`);
}
...
const LIVE_PID_FILE = path.join((await import('node:os')).default.tmpdir(), 'venustas-live.json');
...
async function liveCli() {
  ...
  if (helpMode) {
    console.log(`Usage: venustas live [options]
...
The server provides:
  ...
  server.listen(port, '127.0.0.1', () => {
    ...
    console.log(`Venustas live detection server running on ${url}\n`);

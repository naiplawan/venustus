...
describe('download file paths', () => {
  test('maps opencode skills into the .opencode config directory', () => {
    expect(getFilePath('skill', 'opencode', 'venustus')).toBe(
      path.join(process.cwd(), 'dist', 'opencode', '.opencode', 'skills', 'venustus', 'SKILL.md')
    );
  });
  ...
  test('rejects bundle-only providers on the individual download route', async () => {
    const response = await handleFileDownload('skill', 'universal', 'venustus');

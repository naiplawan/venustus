/**
 * End-to-end tests for `venustus skills` subcommands.
 *
...
function createFakeSkills(root, skills = ['audit', 'polish', 'venustus'], providers = ['.claude']) {
  for (const provider of providers) {
    for (const skill of skills) {
      ...
      writeFileSync(join(skillDir, 'SKILL.md'), [
        ...
        'Use the venustus skill for setup.',
      ].join('\n'));
    }
  }
}
...
describe('skills install: already-installed detection', () => {
  test('detects venustus sentinel and bails', () => {
  ...
  test('detects prefixed i-venustus', () => {
    const tmp = mkdtempSync(join(tmpdir(), 'imp-test-'));
    execSync('git init', { cwd: tmp });

    const skillDir = join(tmp, '.cursor', 'skills', 'i-venustus');
    mkdirSync(skillDir, { recursive: true });
    writeFileSync(join(skillDir, 'SKILL.md'), '---\nname: i-venustus\n---\n');
...
describe('skills install: prefix rename', () => {
  ...
  beforeAll(() => {
    ...
    createFakeSkills(tmp, ['audit', 'polish', 'venustus'], ['.claude', '.cursor']);
  });
...
console.log(JSON.stringify({ count }));
    ...
    expect(skills).toContain('i-venustus');
  ...
  test('prefixed SKILL.md has correct name and cross-references', () => {
    ...
    expect(content).toContain('the i-venustus skill');
...
describe('skills update: direct download fallback', () => {
  ...
  beforeAll(() => {
    ...
    for (const skill of ['audit', 'venustus']) {
...
describe('prefix round-trip: detect, undo, re-apply', () => {
  ...
  beforeAll(() => {
    ...
    for (const skill of ['audit', 'polish', 'venustus']) {
      ...
      writeFileSync(join(skillDir, 'SKILL.md'), [
        ...
        'Use the i-venustus skill for setup.',
      ].join('\n'));
    }
  });
...
for (const d of DIRS) {
  ...
  for (const name of readdirSync(dir)) {
    if (name === 'venustus') { console.log(''); process.exit(); }
    if (name.endsWith('-venustus')) { console.log(name.slice(0, -'venustus'.length)); process.exit(); }
  }
}
console.log('');
    `);
...
console.log(JSON.stringify(readdirSync(skillsDir)));
    ...
    expect(output).toContain('venustus');
    ...
    expect(content).toContain('the venustus skill');
    expect(content).not.toContain('/i-audit');
    expect(content).not.toContain('i-venustus');
  });
...
console.log(JSON.stringify(readdirSync(skillsDir)));
    ...
    expect(output).toContain('i-venustus');

...
Audit the code.`;
  ...
  test('should read skill with reference files', () => {
    const skillContent = `---
name: venustus
description: Venustus design skill
---

Venustus design instructions.`;

    const skillDir = path.join(testRootDir, 'source/skills/venustus');
    ensureDir(skillDir);
    fs.writeFileSync(path.join(skillDir, 'SKILL.md'), skillContent);
...
**DON'T**: Use pure black.`;
  ...
  test('should handle missing SKILL.md file', () => {
    ensureDir(path.join(testRootDir, 'source/skills/venustus'));
...
describe('replacePlaceholders', () => {
  ...
  test('should exclude venustus from {{available_commands}}', () => {
    const result = replacePlaceholders('Commands: {{available_commands}}', 'claude-code', ['audit', 'venustus', 'polish']);
    expect(result).toBe('Commands: /audit, /polish');
  });

  test('should exclude i-venustus from {{available_commands}}', () => {
    const result = replacePlaceholders('Commands: {{available_commands}}', 'claude-code', ['i-audit', 'i-venustus', 'i-polish']);
    expect(result).toBe('Commands: /i-audit, /i-polish');
  });

  test('should exclude legacy teach-venustus from {{available_commands}}', () => {
    const result = replacePlaceholders('Commands: {{available_commands}}', 'claude-code', ['audit', 'teach-venustus', 'polish']);
    expect(result).toBe('Commands: /audit, /polish');
  });

  test('should exclude legacy i-teach-venustus from {{available_commands}}', () => {
    const result = replacePlaceholders('Commands: {{available_commands}}', 'claude-code', ['i-audit', 'i-teach-venustus', 'i-polish']);
    expect(result).toBe('Commands: /i-audit, /i-polish');
  });
...
describe('prefixSkillReferences', () => {
  ...
  test('should not partially match longer skill names', () => {
    const result = prefixSkillReferences('Run /teach-venustus command.', 'i-', ['teach', 'teach-venustus']);
    expect(result).toBe('Run /i-teach-venustus command.');
  });
